...
SRA_human_metaInfo_20200301 <-
  ...
  mutate(body_site = case_when(
    str_detect(KEYWORDS, "stool|feces") ~ "feces",
    str_detect(KEYWORDS, "antecubital|fossa|skin") ~ "skin",
    str_detect(KEYWORDS, "vaginal|vagina") ~ "vagina",
    str_detect(KEYWORDS, "saliva|tongue") ~ "saliva",
    str_detect(KEYWORDS, "semen|sperm") ~ "semen",
    str_detect(KEYWORDS, "nostril|nares") ~ "nostril",
    str_detect(KEYWORDS, "menstrual blood|menstruation") ~ "menstrual_blood",
    TRUE ~ NA_character_))
...
# not-so-random sampling (based on body-site) to create smaller test-set
...
keyword_summary %>%
  filter(n > 1) %>%
  write_csv(here("tasks", "04_cluster_keywords_hclust", "keyword_summary_hclust.csv"))
