...
keyword_distance <-
  function(ID_1, ID_2){
    token_df <- SRA_human_metaInfo_20200301_tokens
...
# usage example
#keyword_distance("ERS001113", "ERS1083355")
```
...
dist_sample <-
  ...
  rowwise() %>%
  mutate(dist = keyword_distance(ID_first, ID_second)) %>%
...
```{r levenshtein-distance-matrix}
# also called edit distance

adist("city;forehead;human;human-skin;raw;rural;skin",
      "female;human;human-skin;skin")

edit_dist <- adist(SRA_human_metaInfo_20200301_sample_site$KEYWORDS)
  
rownames(edit_dist) <- paste(SRA_human_metaInfo_20200301_sample_site$SAMPLE_ID,
                             SRA_human_metaInfo_20200301_sample_site$body_site)
colnames(edit_dist) <- paste(SRA_human_metaInfo_20200301_sample_site$SAMPLE_ID,
                             SRA_human_metaInfo_20200301_sample_site$body_site)
...
plot(clust_sample); rect.hclust(clust_sample, k = 5)
...
```{r k-medioid-clustering}
k_clust_sample <- pam(dist_sample, k = 3)

k_clust_sample$clustering

summary(k_clust_sample)

plot(k_clust_sample)

clusplot(k_clust_sample)
